...
class MultiDAE(object):
    ...
    def build_graph(self):
        ...
        neg_ll = - \
            tf.reduce_mean(tf.reduce_sum(log_softmax_var * self.input_ph, axis=1))
        # apply regularization to weights，lam=0.01
...
class MultiVAE(MultiDAE):
    ...
    def q_graph(self):
        ...
        for i, (w, b) in enumerate(
                zip(self.weights_q, self.biases_q)):  
            h = tf.matmul(h, w) + b  
    ...
    def forward_pass(self):
        ...
        epsilon = tf.random_normal(tf.shape(std_q))  # epsilon
        # zu = E(q) + epsilon*STD(q)*self.is_training_ph
        sampled_z = mu_q + self.is_training_ph * epsilon * std_q

        # p-network
        logits = self.p_graph(sampled_z)  # x_pre

        return tf.train.Saver(), logits, KL  # x_pre & KL
